<img src="SOORAJ SHET GPB.png" width="1200" />

# Sooraj Shet
### Backend Developer · PHP · Laravel · SQL Server

I build reliable enterprise workflow systems with a focus on clean architecture, performance, and scalability.

---

### 🛠 Tech Stack

**Backend**
![](https://skillicons.dev/icons?i=php,laravel,python)

**Frontend**
![](https://skillicons.dev/icons?i=js,html,css)

**Databases & Tools**
![](https://skillicons.dev/icons?i=mysql,git,github,postman,vscode)
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/microsoftsqlserver/microsoftsqlserver-plain-wordmark.svg" height="48" title="SQL Server"/>

---

### 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Dev-Sooraj-Git&show_icons=true&theme=tokyonight&hide_border=true&count_private=true&hide=issues)&nbsp;&nbsp;![Top Repos](https://github-readme-stats.vercel.app/api/top-langs/?username=Dev-Sooraj-Git&layout=compact&theme=tokyonight&hide_border=true&hide=jupyter+notebook)

---

### 📈 Currently Working On
- Strengthening system design fundamentals
- Building testable, scalable backend code
- Exploring clean architecture patterns in PHP

---

### 🤝 Let's Connect
💼 Open to Backend Developer opportunities · [LinkedIn](https://www.linkedin.com/in/sooraj-shet-ai-cse/)
